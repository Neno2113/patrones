...
class USATaxStrategy implements TaxStrategy {
  // Implementar el método calculateTax = amount * 0.1
  calculateTax(amount: number): number {
    return amount * 0.1;
  }
}
...
class CanadaTaxStrategy implements TaxStrategy {
  // Implementar el método calculateTax = amount * 0.13
  calculateTax(amount: number): number {
    return amount * 0.13;
  }
}
...
class GermanyTaxStrategy implements TaxStrategy {
  // Implementar el método calculateTax = amount * 0.19
  calculateTax(amount: number): number {
    return amount * 0.19;
  }
}
...
class TaxCalculator {
  ...
  // Implementar el constructor recibiendo la estrategia a usar
  constructor( stategry: TaxStrategy){
    this.strategy = stategry;
  }
