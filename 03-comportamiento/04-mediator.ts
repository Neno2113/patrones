...
class ChatRoom {
    ...
    sendMessage(sender: User, message: string): void {
        ...
        console.log(`${sender} envió: ${message}`);
    }
...
class User {
    ...
    sendMessage(message: string): void {
        console.log(`%c${this.username} %cenvia: ${message}`, COLORS.blue, COLORS.white);
...
function main(){
    ...
    const user1 = new User('Juan', chatRoom);
    const user2 = new User('Pedro', chatRoom);
    const user3 = new User('Maria', chatRoom);

    user1.sendMessage('Hola a todos');
    user2.sendMessage('Hola Juan, como estas?');
    user3.sendMessage('Hola como estan, fernando, Gaston');
