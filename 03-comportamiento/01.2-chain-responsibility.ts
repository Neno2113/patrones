...
class Supervisor extends BaseApprover {
  override approveRequest(amount: number): void {
    if(amount <= 1000){
      console.log(`Solicitud de %c${amount} aprobada por el Supervisor`, COLORS.green);
    } else {
      console.log(`Solicitud de %c${amount} necesitan ser aprobadas por el Manager`, COLORS.red);
      this.next(amount);
    }
  } 
}
...
class Manager extends BaseApprover {
  ...
  override approveRequest(amount: number): void {
    if(amount <= 5000){
      console.log(`Solicitud de %c${amount} aprobada por el Manager`, COLORS.green);
    } else {
      console.log(`Solicitud de %c${amount} necesitan ser aprobadas por el Director`, COLORS.red);
      this.next(amount);
    }
  }
}
...
class Director extends BaseApprover {
  ...
  override approveRequest(amount: number): void {
    if(amount > 5000){
      console.log(`Solicitud de %c${amount} aprobada por el Director`, COLORS.green);
    } else {
      this.next(amount);
    }
  }
